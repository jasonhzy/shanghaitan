...
jq.fn.banqh = function(can){
	...
	var picnum = jq(can.pic).find('ul li').length;//大图图片数量
	var picw = jq(can.pic).find('ul li').outerWidth(true);//大图图片宽度
	var pich = jq(can.pic).find('ul li').outerHeight(true);//大图图片高度
	var poppicw = jq(can.pop_pic).find('ul li').outerWidth(true);//弹出框框架宽度
	var picminnum = jq(can.pnum).find('ul li').length;//小图图片数量
	var picpopnum = jq(can.pop_pic).find('ul li').length;
	var picminw = jq(can.pnum).find('ul li').outerWidth(true);//小图图片宽度
	var picminh = jq(can.pnum).find('ul li').outerHeight(true);//小图图片高度
	var pictime;
	var tpqhnum=0;
	var xtqhnum=0;
	var popnum=0;
