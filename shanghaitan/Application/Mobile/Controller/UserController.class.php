...
class UserController extends MobileBaseController {
    ...
    public function recharge(){
        ...
        $show = $Page->show();
        $recharge_list = M('recharge')->where(array('user_id'=>$this->user_id,'pay_status'=>1))->order("order_id desc")->limit($Page->firstRow.','.$Page->listRows)->select();
   ...
   */
    public function myMessage(){
        ...
        if($user_id == 0){
            ...
            $this->error('您还没有登录，请登录', U('Mobile/User/login'));
        }
 ...
 */
    ...
    public function info(){
       /* //调用摄像头
        vendor('WinXin.jssdk#class');
        $jssdk = new \Vendor\WinXin\JSSDK("wxc3589940cbd4c6bc", "4c206c50dfa25d96f223abe8f835a2a6");
        $signPackage = $jssdk->getSignPackage("http://cnceshi.com".$_SERVER[REQUEST_URI]);*/
        ...
        if(IS_POST){
            ...
            if(!$userLogic->update_info($this->user_id,$post)){
               ...
               echo  "<script>alert('保存成功');window.location='/index.php?m=Mobile&c=User&a=index';</script>";                      
                exit;
            }
    ...
    public function headImg(){
        ...
        $file = "./Public/upload/head_pic/";
//方式二：先转换为GD图像资源，再生成文件或显示输出
        $im = imagecreatefromstring($img); //resource(2) of type (gd) 图像资源
        imagejpeg ($im,$file.$tmp_file); //图像流（image）以 JPEG 格式输出到标准输出(浏览器或者文件)
        $head_pic = '/Public/upload/head_pic/'.$tmp_file;
        M('users')->where(array('user_id'=>$user_id))->save(array('head_pic'=>$head_pic));
        //move_uploaded_file($img,$file);
        /*$config = array(
            'maxSize'=>3145728,
    ...
    * 修改用户头像
    */
   /* public function mysavehead(){
        $user_id = session("user_id");
        $headimg['head_pic'] = $this->getmedia($this->access_token(),$_POST['serverId']);
        $headimg['head_pic'] = "/".$headimg['head_pic'];
        M("users")->where(array("user_id"=>$user_id))->save($headimg);
        echo(json_encode("1"));
    }*/
    /*
     * 邮箱验证
        ...
        if(IS_POST){
            ...
            if($info['email'] == $email && $info['code'] == $code){
                ...
                if(!$userLogic->update_email_mobile($email,$this->user_id))
                    $this->error('邮箱已存在');
                $this->success('绑定成功',U('Mobile/User/index'));
                exit;
            }
            $this->error('邮箱验证码不匹配');
        }
    ...
    */
    ...
    public function password(){
        ...
        if($user['mobile'] == ''&& $user['email'] == '')
            $this->error('请先绑定手机或邮箱',U('Mobile/User/info'));
        if(IS_POST){
            $userLogic = new UsersLogic();
    ...
    public function forget_pwd(){
    	...
    	if(IS_POST){
    		...
    		$this->success($data['msg'],U('Mobile/User/login'));
    		exit;
    	}
        $this->display();
    }
...
*/
   /* public function getmedia($access_token,$media_id,$foldername){
        ...
        if (!file_exists("Public/upload/weixin/".$foldername)) {
            mkdir("Public/upload/background/".$foldername, 0777, true);
        }
        $targetName = 'Public/upload/weixin/'.$foldername.date('YmdHis').rand(1000,9999).'.jpg';
   ...
   /* public function access_token(){
        vendor('WinXin.jssdk#class');
        $jssdk = new \Vendor\WinXin\JSSDK(" wx11766ee89be34c57", "f0f2696337ac609df40f420c187eef27");
