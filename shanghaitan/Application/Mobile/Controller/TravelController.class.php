...
use Think\Page;
use Think\Verify;
class TravelController extends MobileBaseController {
    ...
    public function orderDetail(){
        if($_GET['tid']){
            ...
            if($_POST['user_id'] == 0) {
                $this->error('你还没有登录，请登录', U("Mobile/User/login"));
            }else {
                //班期id
                $id = $_POST['id'];
                // var_dump($_POST);exit;
    ...
    public function travelList(){
        ...
        $hot_arr = M('travel')->where(array('is_hot'=>1))->order("travel_id desc")->limit(5)->select();
