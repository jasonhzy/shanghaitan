 ...
 * @package Home\Logic
 */
class TravelLogic extends RelationModel
{
     ...
     * @return  mix
     */
    public function travel_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                global $travel_category, $travel_category2;
                $sql = "SELECT * FROM  __PREFIX__travel_category ORDER BY parent_id , sort_order ASC";
                $travel_category = D('travel_category')->query($sql);
                $travel_category = convert_arr_key($travel_category, 'id');
                
                foreach ($travel_category AS $key => $value)
                {
                    if($value['level'] == 1)
                        $this->get_cat_tree($value['id']);                                
                }
                return $travel_category2;
    }
    ...
    {
        global $travel_category, $travel_category2;
        $travel_category2[$id] = $travel_category[$id];
        foreach ($travel_category AS $key => $value){
             if($value['parent_id'] == $id)
             {                 
                $this->get_cat_tree($value['id']);  
                $travel_category2[$id]['have_son'] = 1; // 还有下级
             }
        }               
    }
    ...
    {            
        $travelCategory = M("travelCategory"); // 实例化User对象
        $cat = $travelCategory->where("id = $id")->find(); // 找出他自己
        // 刚新增的分类先把它的值重置一下
        if($cat['parent_id_path'] == '')
        {
            ($cat['parent_id'] == 0) && $travelCategory->execute("UPDATE __PREFIX__travel_category set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类
            $travelCategory->execute("UPDATE __PREFIX__travel_category AS a ,__PREFIX__travel_category AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");
            $cat = $travelCategory->where("id = $id")->find(); // 从新找出他自己
        }        
        ...
        if($cat['parent_id'] == 0) //有可能是顶级分类 他没有老爸
        {
            ...
            $parent_cat = $travelCategory->where("id = {$cat['parent_id']}")->find(); // 找出他老爸的parent_id_path
        }        
        ...
        $travelCategory->execute("UPDATE `__PREFIX__travel_category` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level) WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");
    }    
    ...
    function find_parent_cat($cat_id)
    {
        ...
        $cat_list =  M('travel_category')->getField('id,parent_id,level');
    ...
    {
        $categoryList =  M("travelCategory")->getField('id,name,parent_id,level');
        $nameList = array();
     ...
     * @param int $travel_id
     */
    public function afterSave($travel_id)
    {
        // 商品图片相册  图册
        if(count($_POST['travel_images']) > 1)
        {
            array_unshift($_POST['travel_images'],$_POST['original_img']); // 商品原始图 默认为 相册第一张图片
            array_pop($_POST['travel_images']); // 弹出最后一个
            $travelImagesArr = M('Travel_images')->where("travel_id = $travel_id")->getField('img_id,image_url'); // 查出所有已经存在的图片

            // 删除图片
            foreach($travelImagesArr as $key => $val)
            {
                if(!in_array($val, $_POST['travel_images']))
                    M('Travel_images')->where("img_id = {$key}")->delete(); // 删除所有状态为0的用户数据
            }
            // 添加图片
            foreach($_POST['travel_images'] as $key => $val)
            {
                if($val == null)  continue;
                if(!in_array($val, $travelImagesArr))
                {
                    $data = array(
                        'travel_id' => $travel_id,
                        'image_url' => $val,
                    );
                    M("Travel_images")->data($data)->add();; // 实例化User对象
                }
            }
        }
