 ...
 * @package Home\Logic
 */
class LifeLogic extends RelationModel
{
     ...
     * @return  mix
     */
    public function life_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                global $life_category, $life_category2;
                $sql = "SELECT * FROM  __PREFIX__life_category ORDER BY parent_id , sort_order ASC";
                $life_category = D('life_category')->query($sql);
                $life_category = convert_arr_key($life_category, 'id');
                
                foreach ($life_category AS $key => $value)
                {
                    if($value['level'] == 1)
    ...
    {
        global $life_category, $life_category2;
        $life_category2[$id] = $life_category[$id];
        foreach ($life_category AS $key => $value){
             if($value['parent_id'] == $id)
             {                 
                $this->get_cat_tree($value['id']);  
                $life_category2[$id]['have_son'] = 1; // 还有下级
             }
        }               
    }
    ...
    {            
        $lifeCategory = M("lifeCategory"); // 实例化User对象
        $cat = $lifeCategory->where("id = $id")->find(); // 找出他自己
        // 刚新增的分类先把它的值重置一下
        if($cat['parent_id_path'] == '')
        {
            ($cat['parent_id'] == 0) && $lifeCategory->execute("UPDATE __PREFIX__life_category set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类
            $lifeCategory->execute("UPDATE __PREFIX__life_category AS a ,__PREFIX__life_category AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");
            $cat = $lifeCategory->where("id = $id")->find(); // 从新找出他自己
        }        
        ...
        if($cat['parent_id'] == 0) //有可能是顶级分类 他没有老爸
        {
            ...
            $parent_cat = $lifeCategory->where("id = {$cat['parent_id']}")->find(); // 找出他老爸的parent_id_path
        }        
        ...
        $lifeCategory->execute("UPDATE `__PREFIX__life_category` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level) WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");
    }    
    ...
    function find_parent_cat($cat_id)
    {
        ...
        $cat_list =  M('life_category')->getField('id,parent_id,level');
    ...
    {
        $categoryList =  M("lifeCategory")->getField('id,name,parent_id,level');
        $nameList = array();
