...
class ArticleCatLogic extends RelationModel
{
    ...
    public function activity_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
        static $res = NULL;
        ...
        if ($res === NULL)
        {
            ...
            if ($data === false)
            {
                $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children FROM __PREFIX__activity_cat AS c".
                    " LEFT JOIN __PREFIX__activity_cat AS s ON s.parent_id=c.cat_id".
                    " GROUP BY c.cat_id  ORDER BY parent_id, sort_order";
                $res = D('activity_cat')->query($sql);
                //write_static_cache('art_cat_pid_releate', $res);
            }
            else
            {
        ...
        $options = $this->activity_cat_options($cat_id, $res); // 获得指定分类下的子分类的数组
    ...
    public function activity_cat_options($spec_cat_id, $arr)
    {
        static $cat_options = array();
