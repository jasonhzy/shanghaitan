...
class MessageController extends BaseController {
    ...
    public function detail(){
        $id = I('get.id');
        $res = M('message')->where(array('message_id'=>$id))->find();
        if(!$res){
            exit($this->error('不存在留言'));
        }
        if(IS_POST){
            $add['parent_id'] = $id;
        ...
        $reply = M('message')->where(array('parent_id'=>$id))->select(); // 评论回复列表
    ...
    public function op(){
        ...
        if(!in_array($type,array('del','show','hide')) || !$selected_id)
            $this->error('非法操作');
        $where = "message_id IN ({$selected_id})";
        if($type == 'del'){
            //删除回复
            $where .= " OR parent_id IN ({$selected_id})";
            $row = M('message')->where($where)->delete();
//            exit(M()->getLastSql());
        }
        if(!$row)
            $this->error('操作失败');
    ...
    public function ajaxindex(){
        ...
        $show = $Page->show();
        $comment_list = $model->where($where)->order('time DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
