                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <table id="list-table" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                                    ...
                                    <tbody>
                                    <?php if(is_array($userInfo)): $k = 0; $__LIST__ = $userInfo;if( count($__LIST__)==0 ) : echo "暂无未审核用户" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr role="row">
                                            <td><?php echo ($vo["name"]); ?></td>
                                            <td><?php echo ($vo["nickname"]); ?></td>
                                            <td><?php if($vo["sex"] == 1): ?>男<?php elseif($vo["sex"] == 2): ?>女<?php else: ?>保密<?php endif; ?></td>
                                            <td><?php if($vo["head_pic"] == ''): ?>暂无头像<?php else: ?><img src="<?php echo ($vo["head_pic"]); ?>" width="80px"><?php endif; ?></td>
...
<script>
    $(".static").click(function(){
        ...
        if(con == true){
            $.get("<?php echo U('Admin/User/ajaxCheck');?>",{user_id:user_id},function(data){
                if(data == "1"){
                    location.reload();
                }
            })
