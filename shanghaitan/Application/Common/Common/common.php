 ...
 function sendTemplateSMS($to,$datas,$tempId)
{
    ...
    include_once(APP_PATH."Common/common/CCPRestSmsSDK.php");
    ...
    $appId = '8aaf070858cd982e0158d7bd5abd0723';
 ...
 * @return bool
 */
function sendSMS($mobile,$code,$id)
{
 ...
 /*$data = array
    (
        ...
        'content'=>$code,
       // 'ip'=>$_SERVER['REMOTE_ADDR']//内容
    );*/
    //var_dump($_SERVER['REMOTE_ADDR']);exit;
    //即时发送
    $data = array($code,'2');
    $res = sendTemplateSMS($mobile,$data,$id);//POST方式提交
...
function    getCatGrandson ($cat_id)
{
    ...
    // 把整张表找出来
    $GLOBALS['category_id_arr'] = M('goods_category')->getField('id,parent_id');
    // 先把所有儿子找出来
...
function excart_goods_num($user_id = 0,$session_id = '')
{
    // 查找购物车数量
    $cart_count =  M('excart')->where("user_id = $user_id or session_id = '$session_id'")->sum('goods_num');
    $cart_count = $cart_count ? $cart_count : 0;
    return $cart_count;
}
...
function update_pay_status($order_sn,$pay_status = 1)
{
    if (stripos($order_sn, 'recharge') !== false) {
        //用户在线充值
        $count = M('recharge')->where(array('order_sn'=>$order_sn,'pay_status'=>0))->count();   // 看看有没已经处理过这笔订单  支付宝返回不重复处理操
        if ($count == 0) return false;
        $order = M('recharge')->where(array('order_sn'=>$order_sn))->find();
         M('recharge')->where(array('order_sn'=>$order_sn))->save(array('pay_status' => 1, 'pay_time' => time()));

        accountLog($order['user_id'], $order['account'], 0, '会员在线充值');
    } else {
        // 如果这笔订单已经处理过了
        $count = M('order')->where(array('order_sn'=>$order_sn,'pay_status'=>0))->count();   // 看看有没已经处理过这笔订单  支付宝返回不重复处理操作
        if ($count == 0) return false;
        // 找出对应的订单
        $order = M('order')->where(array('order_sn'=>$order_sn))->find();
        // 修改支付状态  已支付
        M('order')->where(array('order_sn'=>$order_sn))->save(array('pay_status' => 1, 'pay_time' => time()));
        // 减少对应商品的库存
 ...
 */
function cal_price($user_id=0,$order_goods,$shipping_code='',$shipping_price=0,$province=0,$city=0,$district=0)
{
   ...
   /* $coupon_price = 0;
    ...
    $result = array(
        ...
        'market_price'       => $market_price, // 商品总价
...
function call_price($user_id=0,$order_goods,$shipping_code='',$shipping_price=0,$province=0,$city=0,$district=0)
{
    $cartLogic = new \Home\Logic\CartLogic();
    $user = M('users')->where("user_id = $user_id")->find();// 找出这个用户
