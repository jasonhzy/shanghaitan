...
class PaymentController extends BaseController {
    ...
    public function getCode(){     
        ...
        public function getPay(){
            ...
            $code_str = $this->payment->getImage($order,$config_value);
            //微信JS支付
            if($this->pay_code == 'weixin' && $_SESSION['openid'] && strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger')){
                $code_str = $this->payment->getJSAPI($order,$config_value);
                exit($code_str);
    ...
    public function returnUrl(){
        ...
        if(stripos($result['order_sn'],'recharge') !== false)
        {
            ...
            if($result['status'] == 1)
                echo "<script>window.location='/index.php?m=Home&c=User&a=vipPay'</script>";
            else
                echo "<script>window.location='/index.php?m=Home&c=User&a=vipPay'</script>";
            exit();
        }
        ...
        $this->assign('order', $order);
        if($result['status'] == 1)
            echo "<script>window.location='/index.php?m=Home&c=User&a=order_detail&id='+$id</script>";
        else
            echo "<script>window.location='/index.php?m=Home&c=User&a=order_detail&id='+$id</script>";
    }
}
