...
class CartLogic extends RelationModel
{
     ...
     */
    function addexCart($goods_id,$goods_num,$session_id,$user_id)
    {
        $goods = M('Goods')->where("goods_id = $goods_id")->find(); // 找出这个商品
        $arr = M('users')->where(array('user_id'=>$user_id))->find();
        $pay_points = $arr['pay_points'];
        $exchange = $goods['exchange_integral'];
        $total = $exchange*$goods_num;
        $where = " session_id = '$session_id' ";
        $user_id = $user_id ? $user_id : 0;
        if($user_id)
            $where .= "  or user_id= $user_id ";
        $catr_count = M('excart')->where($where)->count(); // 查找购物车商品总数量
        if($user_id == 0){
            return array('status'=>-6,'msg'=>'你还没有登录，请先登录','result'=>'');
        }
        if($total > $pay_points)
            return array('status'=>-5,'msg'=>'您的积分不够，不能兑换此商品','result'=>'');
        if($catr_count >= 20)
        ...
        //$catr_goods = M('excart')->where($where)->find(); // 查找购物车是否已经存在该商品
        $data = array(
            ...
            'store_count'     => $goods['store_count'],//商品库存
            'goods_price'     => $goods['exchange_integral'],  // 购买价
      ...
      /*  if($catr_goods)
        {
            M('excart')->where("user_id = $user_id ")->delete();
            $num = $catr_goods['goods_num'] + $goods_num;
            $tnum = $num * $catr_goods['goods_price'];
            if($tnum > $pay_points)
                return array('status'=>-5,'msg'=>'您的积分不够，不能兑换此商品','result'=>'');
            $result = M('excart')->where("id =".$catr_goods[id])->save(  array("goods_num"=> ($catr_goods['goods_num'] + $goods_num)) ); // 数量相加
            $cart_count = excart_goods_num($user_id,$session_id); // 查找购物车数量
            return array('status'=>1,'msg'=>'成功加入购物车','result'=>$cart_count);
        }
        else
        {*/
            //$tot = $data['goods_num']*$data['goods_price'];
            M('excart')->where("user_id = $user_id ")->delete();
            $insert_id = M('excart')->add($data);
            $cart_count = excart_goods_num($user_id,$session_id); // 查找购物车数量
            return array('status'=>1,'msg'=>'成功加入购物车','result'=>$cart_count);
        $cart_count = excart_goods_num($user_id,$session_id); // 查找购物车数量
        return array('status'=>-5,'msg'=>'加入购物车失败','result'=>$cart_count);
    }
    function addCart($goods_id,$goods_num,$goods_spec,$session_id,$user_id)
    {
        $goods = M('Goods')->where("goods_id = $goods_id")->find(); // 找出这个商品
        if($goods_spec){
            $specGoodsPriceList = M('goods_attr')->where("goods_attr_id = $goods_spec")->find(); // 获取商品对应的规格价钱 库存 条码
            $color = $specGoodsPriceList['attr_value'];
        }else{
            $color="随机";
        }
		$where = " session_id = '$session_id' ";
       ...
       /* if(empty($specGoodsPriceList)) // 有商品规格 但是前台没有传递过来
            return array('status'=>-1,'msg'=>'必须传递商品颜色','result'=>'');*/
        if($goods_num == 0) 
            return array('status'=>-2,'msg'=>'购买商品数量不能为0','result'=>'');
        if($user_id == 0){
            return array('status'=>-6,'msg'=>'你还没有登录，请先登录','result'=>'');
...
/*        foreach($goods_spec as $key => $val) // 处理商品规格
        ...
        $data = array(                    
                    ...
                    'spec_key_name'   => $color, // 规格 key_name
     ...
     */
    function cartList($user = array() , $session_id = '',$mode =0)
    {                   
        ...
        $total_price = array(
            ...
            'anum'=>$result['anum'],
            'shop_total'=> $result['goods_price'],
            'market_total'=> $result['market_price']
           ); // 总计
    ...
    function carList($user = array() , $session_id = '',$mode =0)
    {
        ...
        $cartList = M('excart')->where($where)->select();  // 获取购物车商品
        //var_dump($cartList);exit;
        $data = call_price($user[user_id],$cartList);
           //var_dump($data);exit;
        if($data['status'] < 0)
        {
            if($mode) return array();
...
function cart_freight2($shipping_code,$province,$city,$district,$weight)
{
   ...
   return $freight;  
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price)
    {
        ...
        // 1插入order_goods 表
        $cartList = M('Cart')->where("user_id = $user_id")->select();
    ...
    public function addexOrder($user_id,$address_id,$car_price)
    {

        // 仿制灌水 1天只能下 50 单  // select * from `tp_order` where user_id = 1  and order_sn like '20151217%'
        $order_count = M('exchange')->where("user_id= $user_id and order_sn like '".date('Ymd')."%'")->count(); // 查找购物车商品总数量
        if($order_count >= 20)
            return array('status'=>-9,'msg'=>'一天只能兑换20单','result'=>'');
        // 0插入订单 order
        $address = M('UserAddress')->where("address_id = $address_id")->find();
        $data = array(
            'order_sn'         => date('YmdHis').rand(1000,9999), // 订单编号
        ...
        $order_id = M("exchange")->data($data)->add();
        if(!$order_id)
            return array('status'=>-8,'msg'=>'添加订单失败','result'=>NULL);

        // 1插入order_goods 表
        $cartList = M('excart')->where("user_id = $user_id")->select();
        foreach($cartList as $key => $val)
        {
            $data2['order_id'] = $order_id; // 订单id
            $data2['goods_id'] = $val['goods_id']; // 商品id
            ...
            $data2['market_price'] = $val['market_price']; // 市场价
            $data2['goods_price'] = $val['goods_price']; // 商品价
            $data2['spec_key'] = 0; // 商品规格
            $data2['spec_key_name'] = '随机'; // 商品规格名称
            $order_goods_id = M("exorder_goods")->data($data2)->add();
        }
        // 扣除商品库存  扣除库存移到 付完款后扣除
        M('Goods')->where("goods_id = ".$val['goods_id'])->setDec('store_count',$val['goods_num']); // 商品减少库存

        // 3 扣除积分 扣除余额
        M('Users')->where("user_id = $user_id")->setDec('pay_points',$car_price['goodsFee']); // 用户的积分减
        //var_dump($order_id);exit;
        // 4 清空购物车
        M('excart')->where("user_id = $user_id ")->delete();
        return array('status'=>1,'msg'=>'提交订单成功','result'=>$order_id); // 返回新增的订单id
