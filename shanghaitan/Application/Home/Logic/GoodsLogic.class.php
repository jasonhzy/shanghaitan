...
class GoodsLogic extends RelationModel
{
   ...
   public function get_look_see($goods){
   	  return M('goods')->where("goods_id !=".$goods['goods_id']." and cat_id!=".$goods['cat_id']." and is_on_sale = 0")->limit(3)->select();
   }
    public function get_lookto_see($goods){
        return M('goods')->where("goods_id !=".$goods['goods_id']." and cat_id!=".$goods['cat_id']." and is_on_sale = 1")->limit(3)->select();
    }
   
 /**
 * 帅选的价格期间 
 * @param type $goods_id_str 帅选的分类idto
