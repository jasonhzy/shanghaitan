...
class UsersLogic extends RelationModel
{
     ...
     * @return array
     */
    public function reg($username,$password,$password2,$mobile){
    	$is_validated = 0 ;
    ...
    public function get_order_goods($order_id){
        $sql = "SELECT og.*,g.original_img,g.shop_price FROM __PREFIX__order_goods og LEFT JOIN __PREFIX__goods g ON g.goods_id = og.goods_id WHERE order_id = ".$order_id;
    ...
    public function get_exorder_goods($order_id){
        $sql = "SELECT og.*,g.original_img,g.exchange_integral FROM __PREFIX__exorder_goods og LEFT JOIN __PREFIX__goods g ON g.goods_id = og.goods_id WHERE order_id = ".$order_id;
        $goods_list = $this->query($sql);
...
//        $type = I('get.type',0);
    ...
    public function add_excomment($add){
        ...
        //检查订单是否已完成
        $order = M('exchange')->where("order_id = {$add[order_id]} AND user_id = {$add[user_id]}")->find();
        if($order['order_status'] != 2)
            return array('status'=>-1,'msg'=>'该笔订单还未完成','result'=>'');
        ...
        {
            //更新订单商品表状态
            M('exorder_goods')->where(array('goods_id'=>$add[goods_id],'order_id'=>$add[order_id]))->save(array('is_comment'=>1));
            M('goods')->where(array('goods_id'=>$add[goods_id]))->setInc('comment_count',1); // 评论数加一
            // 查看这个订单是否全部已经评论,如果全部评论了 修改整个订单评论状态
            $comment_count   = M('order_goods')->where("order_id ='{$add[order_id]}' and is_comment = 0")->count();
            if($comment_count == 0) // 如果所有的商品都已经评价了 订单状态改成已评价
            {
                M('exchange')->where("order_id ='{$add[order_id]}'")->save(array('order_status'=>3));
            }
            return array('status'=>1,'msg'=>'评论成功','result'=>'');
        }
        return array('status'=>-1,'msg'=>'评论失败','result'=>'');
    ...
    public function sms_log($mobile,$code,$session_id)
    {
       ...
       // $sms_time_out = tpCache('sms.sms_time_out');
        ...
        $send = sendSMS($mobile, $code, '139095');
        //var_dump($send);exit;
        if ($send) {
    ...
    public function confirm_order($user_id,$id){        
        ...
        order_give($id);// 调用送礼物方法, 给下单这个人赠送相应的礼物
