 ...
 */
function AjaxAddCart(goods_id,num,to_catr,color)
{
	// 如果有商品规格 说明是商品详情页提交
        if($("#buy_goods_form").length > 0){
        	//alert("11");
                ...
                $.ajax({
                        type : "POST",
                        url:"/index.php?m=Home&c=Cart&a=ajaxAddCart",
                        data :{goods_id:goods_id,goods_num:num,color:color} ,
						dataType:'json',
                        success: function(data){
							   if(data.status == -1)
							   {
									location.href = "/index.php?m=Home&c=Goods&a=goodsDetail&id="+goods_id;
							   }
							   else
							   {
								    // 加入购物车有误
 ...
 function AjaxExchange(goods_id,num,to_catr){
	 $.ajax({
		 type : "POST",
		 url:"/index.php?m=Home&c=Cart&a=ajaxExchange",
		 ...
		 success: function(data){
			 ...
			 else
			 {
				 ...
				 if(to_catr == 1)  //直接兑换
				 {
					 location.href = "/index.php?m=Home&c=Cart&a=excart";
				 }else {
					 cart_num = parseInt($('#cart_quantity').html()) + parseInt(num);
					 $('#cart_quantity').html(cart_num)
					 layer.open({
